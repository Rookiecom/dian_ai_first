...
class Conv2d(_ConvNd):
    ...
    def conv2d(self, input, kernel, bias = 0, stride=1, padding=0):
        ...
        input_padded = F.pad(input, (self.padding[0], self.padding[0], self.padding[0], self.padding[0]))
        for b in range(batch_size):
            for c_out in range(out_channels):
                for h_out in range(out_h):
                    for w_out in range(out_w):
                        h_in, w_in = h_out * self.stride[0], w_out * self.stride[0]
                        receptive_field = input_padded[b, :, h_in:h_in + kernel_h, w_in:w_in + kernel_w]
    ...
    def backward(self, ones: Tensor):
        weight_rotated = torch.rot90(self.weight, 2, dims=[2, 3])
        weight_rotated=weight_rotated.transpose(0,1)
        ...
        out_h=(in_h + 2 * self.padding[0] - kernel_h) // self.stride[0] + 1
        out_w = (in_w + 2 * self.padding[1] - kernel_w) // self.stride[1] + 1
        grad_input = torch.zeros(self.input.shape, device=self.input.device, dtype=self.input.dtype)
        grad_weight = torch.zeros(self.weight.shape, device=self.weight.device, dtype=self.weight.dtype)
        grad_bias = torch.zeros(self.bias.shape, device=self.bias.device, dtype=self.bias.dtype)
        for b in range(batch_size):
            for c_out in range(out_channels):
                for h_out in range(out_h):
                    for w_out in range(out_w):
                        h_in, w_in = h_out * self.stride[0], w_out * self.stride[0]
                        grad=ones_padded[b, :, h_in:h_in + kernel_h, w_in:w_in + kernel_w]
                        grad_input[b, c_out, h_out, w_out]=torch.sum(grad*weight_rotated[c_out])
        clone_input=self.input.clone().transpose(0,1)
        clone_ones=ones.clone().transpose(0,1)
        ...
        for b in range(batch_size):
            for c_out in range(out_channels):
                for h_out in range(out_h):
                    for w_out in range(out_w):
                        h_in, w_in = h_out * self.stride[0], w_out * self.stride[0]
                        grad=clone_input[b, :, h_in:h_in + kernel_h, w_in:w_in + kernel_w]
                        grad_weight[b, c_out, h_out, w_out]=torch.sum(grad*clone_ones[c_out])
        grad_weight = grad_weight.transpose(0, 1)
        grad_bias= torch.sum(ones, dim=(0, 2, 3))
...
class Linear(Module):
    ...
    def forward(self, input):
        ...
        return self.output
    def backward(self, ones: Tensor):
        input_grad = torch.matmul(ones, self.weight)
        self.weight.grad = torch.matmul(ones.t(), self.input)
        if hasattr(self, 'bias'):
